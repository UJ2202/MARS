...
class HITLPlanningPhase(Phase):
    ...
    def _extract_plan(self, planning_context, plan_file: str = None) -> List[Dict]:
        ...
        print(f"[DEBUG _extract_plan] raw_plan type: {type(raw_plan)}")
        print(f"[DEBUG _extract_plan] raw_plan preview: {str(raw_plan)[:500] if raw_plan else 'None'}")
        ...
        if not raw_plan:
            ...
            if plan_file:
                try:
                    ...
                    print(f"[DEBUG _extract_plan] Loaded from file: {len(plan_steps_list)} steps")
                    return plan_steps_list
                except (json.JSONDecodeError, FileNotFoundError) as e:
                    print(f"[DEBUG _extract_plan] Failed to load from file: {e}")
            return []
        ...
        if hasattr(raw_plan, 'model_dump'):  # Pydantic v2
            ...
            plan_steps_list = plan_dict.get('sub_tasks', [])
            print(f"[DEBUG _extract_plan] Pydantic v2: {len(plan_steps_list)} steps")
        elif hasattr(raw_plan, 'dict'):  # Pydantic v1
            plan_dict = raw_plan.dict()
            plan_steps_list = plan_dict.get('sub_tasks', [])
            print(f"[DEBUG _extract_plan] Pydantic v1: {len(plan_steps_list)} steps")
        elif isinstance(raw_plan, dict):
            plan_steps_list = raw_plan.get('sub_tasks', raw_plan.get('steps', []))
            print(f"[DEBUG _extract_plan] Dict: {len(plan_steps_list)} steps")
        elif isinstance(raw_plan, list):
            plan_steps_list = raw_plan
            print(f"[DEBUG _extract_plan] List: {len(plan_steps_list)} steps")
            ...
            if plan_file:
                try:
                    with open(plan_file, 'r') as f:
                        plan_dict = json.load(f)
                    plan_steps_list = plan_dict.get('sub_tasks', [])
                    print(f"[DEBUG _extract_plan] String->File: {len(plan_steps_list)} steps")
            ...
            if not plan_steps_list:
                print(f"[DEBUG _extract_plan] String plan not parsed, raw: {raw_plan[:300]}...")
        else:
            plan_steps_list = []
        ...
        print(f"[DEBUG _extract_plan] Final: {len(normalized_steps)} normalized steps")
        if normalized_steps:
            print(f"[DEBUG _extract_plan] First step: {normalized_steps[0]}")
