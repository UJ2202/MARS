    ...
    def enrich_node(self, node_id: str, run_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not run_id:
            ...
            if dag_node:
                ...
                print(f"[DAGMetadata] Auto-detected run_id {run_id} for node {node_id}")
        ...
        if run_id:
            ...
            print(f"[DAGMetadata] Found {len(events)} events for node {node_id} in run {run_id}")
        else:
            # No run_id found - return empty to avoid wrong data
            print(f"[DAGMetadata] ERROR: Could not determine run_id for node {node_id}")
            events = []
